...
def tabtransformer_core_fn(material_outputs, config, task_type, gnn_stage=None):
    ...
    # 定義完整的前向傳播函數
    def forward(tf):
        print(f"type(tf): {type(tf)}")
        print(f"len(tf): {len(tf)}")
        # 編碼階段
        encoded_features = encode_batch(tf)
        # print(f"encoded_features : {encoded_features}")
        print(f"len(encoded_features): {len(encoded_features)}")
        print(f"len(encoded_features[0]): {len(encoded_features[0])}")
        print(f"len(encoded_features[1]): {len(encoded_features[1])}")
        # 列間交互階段
        processed_features = process_batch_interaction(encoded_features)
        # print(f"processed_features: {processed_features}")    
        print(f"len(processed_features): {len(processed_features)}")    
        print(f"len(processed_features[0]): {len(processed_features[0])}")
        print(f"len(processed_features[1]): {len(processed_features[1])}")
        # 準備解碼輸入
        xs = []
        batch_size = len(tf)
        ...
        for feature, feature_type in processed_features:
            if feature_type == "categorical":
                ...
                # 數值特徵已經是展平的
                xs.append(feature)
        # print(f"xs after processing: {xs}")
        print(f"len(xs): {len(xs)}")
        print(f"len(xs[0]): {len(xs[0])}")
        print(f"len(xs[1]): {len(xs[1])}")
        # 連接所有特徵
        if xs:
            x = torch.cat(xs, dim=1)
            # 解碼階段
